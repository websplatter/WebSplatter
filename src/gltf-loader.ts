import { ReorderMethod } from './reorder-types';
import { PLYObject } from './ply-loader';
import { log, error, time, timeLog, showPLYSpinner, hidePLYSpinner, setPLYSpinnerStatus } from './utils/simple-console';
import { PadTo4Bytes } from './utils/util';

const c_size_f16 = 2;
const gs_stride = 10;
const c_size_3d_gaussian = gs_stride * c_size_f16;

export async function loadGltfFromFile(input: File, device: GPUDevice, sh_degree: number, sort: ReorderMethod | null): Promise<PLYObject> {
    log(`loading glTF file from File... : ${input.name}`);
    showPLYSpinner('loading glTF...');
    const arrayBuffer = await input.arrayBuffer();
    try {
        return await processGLBInWorker(arrayBuffer, device, sh_degree, sort);
    } finally {
        hidePLYSpinner();
    }
}

export async function loadGltfFromURL(url: string, device: GPUDevice, sh_degree: number, sort: ReorderMethod | null): Promise<PLYObject> {
    log(`loading glTF file from URL... : ${url}`);
    showPLYSpinner('downloading glTF...');
    try {
        time();
        return await processGLBInWorker({ url }, device, sh_degree, sort);
    } finally {
        hidePLYSpinner();
    }
}

type GLBInput = ArrayBuffer | { url: string };

async function processGLBInWorker(input: GLBInput, device: GPUDevice, sh_degree: number, sort: ReorderMethod | null): Promise<PLYObject> {
    return new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./workers/gltf-worker.ts', import.meta.url), { type: 'module' });

        worker.onmessage = (e: MessageEvent<any>) => {
            const msg = e.data;
            if (msg?.type === 'error') {
                error(`glTF worker error: ${msg.message ?? 'unknown error'}`);
                worker.terminate();
                reject(new Error(msg.message ?? 'Worker error'));
                return;
            } else if (msg?.type === 'download_progress') {
                const totalBytes: number | undefined = msg.totalBytes;
                const loadedMB = msg.loadedBytes / (1024 * 1024);
                const totalMB = totalBytes ? (totalBytes / (1024 * 1024)) : undefined;
                const speedMBs = (msg.speedBps ?? 0) / (1024 * 1024);
                const percent = totalBytes ? Math.min(99, Math.floor((msg.loadedBytes / totalBytes) * 100)) : undefined;
                const totalLine = totalMB ? `total ${totalMB.toFixed(1)} MB` : 'total -- MB';
                const progressLine = totalMB && percent !== undefined
                    ? `${loadedMB.toFixed(1)} MB downloaded (${percent}%)`
                    : `${loadedMB.toFixed(1)} MB downloaded`;
                const speedLine = `${speedMBs.toFixed(2)} MB/s`;
                setPLYSpinnerStatus(`downloading glTF ...\n${totalLine}, ${progressLine}\n${speedLine}`);
                return;
            } else if (msg?.type === 'fetched') {
                log(`Fetched (${msg.byteLength} bytes)`);
                timeLog('Download');
                setPLYSpinnerStatus('parsing glTF...');
                time();
                return;
            } else if (msg?.type === 'buffer_info') {
                log(`Using ${msg.useShared ? 'SharedArrayBuffer' : 'ArrayBuffer'} for buffers`);
                return;
            } else if (msg?.type === 'sorting') {
                const method = msg.method ?? (msg.sort ? `${msg.sort} sorting` : 'sorting');
                const bits = msg.bitsPerAxis ? ` (${msg.bitsPerAxis} bits/axis)` : '';
                const pts = typeof msg.points === 'number' ? ` ${msg.points} splats` : '';
                setPLYSpinnerStatus(`${method}${bits}${pts}\nreordering...`);
                return;
            } else if (msg?.type === 'parse_progress') {
                const total: number = msg.total ?? 0;
                const read: number = msg.read ?? 0;
                const percent = total > 0 ? Math.floor((read / total) * 100) : 0;
                setPLYSpinnerStatus(`parsing glTF ...\n${read}/${total} splats (${percent}%)`);
                return;
            } else if (msg?.type === 'done') {
                const num_points: number = msg.num_points;
                const final_sh_degree: number = msg.final_sh_degree;
                const gs_stride_recv: number = msg.gs_stride;
                const sh_stride_recv: number = msg.sh_stride;

                const gaussian_cpu = new Float16Array(msg.gaussian_cpu);
                const sh_cpu = new Float16Array(msg.sh_cpu);
                log(`Total splats: ${num_points}`);

                const gaussian_3d_buffer = device.createBuffer({
                    label: 'gltf input 3d gaussians data buffer',
                    size: PadTo4Bytes(num_points * c_size_3d_gaussian),
                    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.STORAGE,
                });
                device.queue.writeBuffer(gaussian_3d_buffer, 0, gaussian_cpu);

                const c_size_sh_coef = sh_stride_recv * c_size_f16;
                const sh_buffer = device.createBuffer({
                    label: 'gltf input spherical harmonics data buffer',
                    size: PadTo4Bytes(num_points * c_size_sh_coef),
                    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.STORAGE,
                });
                device.queue.writeBuffer(sh_buffer, 0, sh_cpu);

                worker.terminate();
                timeLog('Parse');

                resolve({
                    num_points,
                    ply_precision: 'f16' as const,
                    gaussian_3d_buffer,
                    sh_buffer,
                    sh_degree: final_sh_degree,
                    gaussian_cpu,
                    sh_cpu,
                    gs_stride: gs_stride_recv,
                    sh_stride: sh_stride_recv,
                });
            }
        };
        worker.onerror = (err) => {
            worker.terminate();
            reject(err);
        };
        if (input instanceof ArrayBuffer) {
            setPLYSpinnerStatus('parsing glTF...');
            worker.postMessage({ type: 'start', glbBuffer: input, sh_degree, sort }, [input]);
        } else {
            worker.postMessage({ type: 'start_url', url: input.url, sh_degree, sort });
        }
    });
}
